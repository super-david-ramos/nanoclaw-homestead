...
## [Unreleased — fork: nanoclaw-homestead] - 2026-05-03

These changes live on the `feat/host-bun-migration` branch and have not been merged to upstream `qwibitai/nanoclaw`. They are specific to the `super-david-ramos/nanoclaw-homestead` fork.

- [BREAKING for fork] **Host runtime moved from Node + pnpm to Bun.** Both host and container now run on Bun. The host's `bun.lock` / `bunfig.toml` / `package.json.trustedDependencies` replace `pnpm-lock.yaml` / `pnpm-workspace.yaml`; supply-chain gate (`install.minimumReleaseAge = 259200`) preserves the prior 3-day pnpm release-age policy. Test runner: `bun test --isolate`. No more `dist/` build step — Bun runs `src/index.ts` directly. See [docs/plan/phases/phase-bun-migration.md](docs/plan/phases/phase-bun-migration.md) for the full migration log, surprises caught, and rollback recipe.
- [BREAKING for fork] **launchd plist points at a `Homestead Nanobot.app` bundle, not at `bun` directly.** macOS Background Task Management attributes a launch item to the developer who code-signed its executable. Pointing the plist at `/opt/homebrew/bin/bun` showed up in System Settings → Login Items as "Jarred Sumner" (bun's signing identity), which made the item easy to disable accidentally and once led to a 2-hour outage when SMD bootout sent SIGTERM. Wrapping bun in a tiny `.app` bundle whose `Info.plist` declares `CFBundleName="Homestead Nanobot"` makes BTM display that name. Bundle generated by `setup/service.ts` at `~/Applications/Homestead Nanobot.app/`.
- [fix for fork] **`writeOutboundDirect` was opening `outbound.db` read-only.** Pre-existing bug surfaced by new router tests; never fired in production because the household is admin-only and only non-admin users hit the deny path. Fix: new `openOutboundDbWrite` helper.
- [fork] **Installer chain rewritten for Bun.** `nanoclaw.sh` + `setup.sh` + `setup/install-bun.sh` (replaces `setup/install-node.sh`) + `setup/probe.sh` + every `setup/install-<channel>.sh` and `setup/add-<channel>.sh` now drive Bun end-to-end. `bun add` replaces `pnpm install` and the separate `pnpm run build` step is gone (Bun runs TS directly).
- **router.ts coverage** — new `src/router.test.ts` with 23 tests covering channel-request gate flows, engage_mode variants, `no_agent_engaged` audit row, command-gate filter+deny, replyTo redirect, setter overwrite warnings, unknown engage_mode default, adapter thread policy, per-thread session-mode override. router.ts line coverage went from ~17% to 97%.

## [2.0.59 — merged from upstream on the bun branch] - 2026-05-12

Upstream history merged into the fork via `git merge upstream/main` on `feat/host-bun-migration`. All upstream changes from `2.0.14` through `2.0.59` come along — bullet list below summarises the ones the merge resolution had to think about.
