...
Known stale post-migration (not yet updated): `nanoclaw.sh` installer still tries to install Node + pnpm; `README_zh.md` and `README_ja.md` still reference Node + pnpm in install requirements. Filed as follow-ups for whoever brings up a fresh Mac Mini deployment.
